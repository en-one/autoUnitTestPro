...
LLM_SUPPPLY_ARGS_PROMPT = """
    ...
    测试代码补充规则:
    1. 只允许补充tt.args.args.Body或tt.args.args.Queries中的参数，不得修改测试框架的其他部分。
    2. 分析被测试函数的逻辑，确定它使用的是args.Body还是args.Queries来获取参数。
    3. 对于正例测试用例，补充必要且有效的参数值到对应的位置。
    4. 对于反例测试用例，根据services层代码中对参数的校验逻辑，构造以下类型的错误用例：
       a. 缺失必需参数
       b. 参数值为空或格式不正确
       c. 特殊分支条件不满足
    5. 仅修改wantReply.Code和wantReply.Result以匹配预期的错误响应，不改变其他结构。
    6. 测试用例的数量应覆盖主要功能路径和异常场景，但不应冗余。
